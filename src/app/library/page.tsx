...
export default function SampleLibraryPage() {
  ...
  useEffect(() => {
    const fetchDocuments = async () => {
      ...
      try {
        ...
        setError('Failed to load documents from corpus.');
        setDocuments([]);
        setTotalDocs(0);
        setTotalPages(0);
      } finally {
  ...
  }, [currentPage, selectedDocType, selectedCategory, selectedJurisdiction, searchQuery, scopeCategories]);
  ...
  const handleZipFileSelect = (file: File) => {
    ...
    if (file.size > 100 * 1024 * 1024) {
      alert('ZIP file size exceeds 100MB limit.');
      return;
    }
    alert(`Successfully selected: ${file.name}`);
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="bg-[#EFF6FF] border border-[#BFDBFE] rounded-lg px-4 py-2 text-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <span>
            ...
            Standardized statutory templates pre-indexed for zero-latency retrieval.
          </span>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6 items-start">
        <div className="lg:col-span-2 space-y-4">
          <div className="space-y-1">
            ...
            <p className="text-xs sm:text-sm text-[#475569] leading-relaxed">
              Access 500 documents including {stats?.templates_count || 200} templates, {stats?.public_laws_count || 285} statutes, and {stats?.demo_documents_count || 15} demos.
            </p>
          </div>
        ...
        <div className="scaffold-card p-5 bg-[#0F172A] text-white border-0 shadow-sm space-y-3">
          ...
          <p className="text-[11px] text-slate-300 leading-relaxed">
            Instant clause-level risk mapping &amp; benchmark cross-examination.
          </p>
        </div>
      </div>
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`px-3 py-1.5 rounded font-medium transition-colors ${
              selectedDocType === 'template' ? 'bg-[#0F172A] text-white' : 'bg-[#F1F5F9] text-[#475569] hover:bg-[#E2E8F0]'
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded font-medium transition-colors ${
              selectedDocType === 'public_law' ? 'bg-[#0F172A] text-white' : 'bg-[#F1F5F9] text-[#475569] hover:bg-[#E2E8F0]'
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded font-medium transition-colors ${
              selectedDocType === 'fictional_demo' ? 'bg-[#0F172A] text-white' : 'bg-[#F1F5F9] text-[#475569] hover:bg-[#E2E8F0]'
            }`}
          >
      ...
      <div className="scaffold-card p-5 border border-[#CBD5E1] bg-[#F8FAFC] flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-start gap-3">
          ...
          <div className="space-y-0.5 text-xs">
            ...
            <p className="text-[#475569] leading-relaxed">Bulk import your enterprise standard forms.</p>
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <button onClick={() => setIsDocsModalOpen(true)} className="px-3 py-1.5 text-xs font-semibold text-[#0F172A] bg-white border border-[#CBD5E1] hover:bg-[#F1F5F9] rounded transition-colors" aria-label="View zero-retention privacy documentation">
            Documentation
          </button>
          <button onClick={openZipFilePicker} className="px-3 py-1.5 text-xs font-semibold text-white bg-[#0284C7] hover:bg-[#0369A1] rounded">
            Batch Intake (.zip)
          </button>
          <input ref={zipFileInputRef} type="file" className="hidden" accept=".zip" aria-label="Upload a ZIP file" onChange={(e) => {const f = e.target.files?.[0]; if (f) handleZipFileSelect(f);}} />
        </div>
      </div>
      ...
      {!isLoading && !error && totalDocs > 0 && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-[#64748B] pt-2">
          <div className="font-mono text-[11px]">
            Showing {Math.min((currentPage - 1) * itemsPerPage + 1, totalDocs)}-{Math.min(currentPage * itemsPerPage, totalDocs)} of {totalDocs} verified {getDocTypeLabel(true)}
          </div>
      ...
      <ZeroRetentionDocsModal isOpen={isDocsModalOpen} onClose={() => setIsDocsModalOpen(false)} />
